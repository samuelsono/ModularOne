...
public class DashboardService(ReportingDbContext dbContext, IReportQueryService reportQueryService) : IDashboardService
{
    public IReadOnlyDictionary<string, string[]>? ValidateDashboard(SaveDashboardRequest request) =>
        ReportValidator.ValidateDashboard(request);

    public IReadOnlyDictionary<string, string[]>? ValidateSection(SaveDashboardSectionRequest request) =>
        ReportValidator.ValidateSection(request);
        ...
        CancellationToken cancellationToken)
    {
                     ...
                     .ThenBy(item => item.Report.Name))
        {
            ...
            try
            {
                data = await reportQueryService.ExecuteAsync(placement.Report.Id, cancellationToken);
            }
