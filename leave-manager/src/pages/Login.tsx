...
import { loginAndStoreSession } from '../services/authService';

const muted = { color: tokens.colorNeutralForeground3 };

function readLoginError(err: unknown): string {
  const axiosData = (err as { response?: { data?: { detail?: string; title?: string; message?: string } } })
    ?.response?.data;
  return (
    axiosData?.detail ||
    axiosData?.message ||
...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      await loginAndStoreSession({ username, password, rememberMe: false });
