    ...
    ILeaveDocumentStorage leaveDocumentStorage) : ILeaveApprovalService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var entityId = await ExecuteInTransactionAsync(async ct =>
        {
            ...
            if (leaveType.DeductsBalance)
            {
                ...
                var balance = await leaveBalanceService.GetOrCreateBalanceAsync(
                    requesterUserId,
                    leaveType.Id,
                    cycleStart,
                    cycleEnd,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var profiles = await dbContext.StaffProfiles
            .AsNoTracking()
            .Where(profile => profile.UserId == userId || profile.UserId == entity.RequesterUserId)
            .ToListAsync(cancellationToken);

        var viewerProfile = profiles.FirstOrDefault(profile => profile.UserId == userId);
        var requesterProfile = profiles.FirstOrDefault(profile => profile.UserId == entity.RequesterUserId);
    ...
    private static LeaveRequestDto Map(LeaveRequest entity) =>
        new(
            entity.Id,
            entity.RequesterUserId,
